---
updated_at: 2025-11-16T17:54:00.769+05:30
edited_seconds: 320
---
...
$$\huge f(x) = f_0 + u \Delta f_0 + \frac{u(u-1)}{2!} \Delta^2 f_0 + \frac{u(u-1)(u-2)}{3!} \Delta^3 f_0$$
...
$$\huge f(x) = f(x_0) + (x - x_0)f[x_0,x_1] + (x - x_0)(x - x_1)f[x_0,x_1,x_2]$$
...
Forward:  
$$\huge f'(x_0) = \frac{1}{h} \Delta f_0 - \frac{1}{2h} \Delta^2 f_0 + \frac{1}{3h} \Delta^3 f_0$$
...
$$\huge\frac{h}{2} \left[f_0 + 2(f_1 + \dots + f_{n-1}) + f_n\right]$$

### Simpson’s 1/3:

$$\huge\frac{h}{3}\left[f_0 + 4(f_1 + f_3 + \dots) + 2(f_2 + f_4 + \dots) + f_n\right]$$

### Simpson’s 3/8:

$$\huge\frac{3h}{8}\left[f_0 + 3(f_1 + f_2 + f_4 + \dots) + 2(f_3 + f_6 + \dots) + f_n\right]$$

---

## **15. Taylor Series Method**

$$\huge y(x+h) = y(x) + h y'(x) + \frac{h^2}{2!} y''(x) + \frac{h^3}{3!} y'''(x)$$
...
Liebmann:  
$$\huge u_{i,j}^{(k+1)} = \frac{1}{4}(u_{i+1,j}^{(k)} + u_{i-1,j}^{(k+1)} + u_{i,j+1}^{(k)} + u_{i,j-1}^{(k+1)})$$
...
$$\huge u_i^{n+1} = (1 - 2\lambda)u_i^n + \lambda(u_{i-1}^n + u_{i+1}^n)$$
